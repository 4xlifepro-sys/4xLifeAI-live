...
function runBacktest(pair: string, h4Raw: Candle[], m5Raw: Candle[]): TradeResult[] {
  ...
  let lastSignalTs = 0;
  const cooldownMs = 60 * 60 * 1000; // 1 hour cooldown (was 4h, too strict)
  const pipMult = getPipMultiplier(pair);
  const pipDigits = pair.includes('JPY') ? 3 : pair.includes('XAU') ? 2 : ['BTC','ETH','LTC','XRP','ADA','SOL','DOGE','BNB'].some(c => pair.includes(c)) ? 0 : 5;

  for (let i = 100; i < m5.length - 10; i++) {
    // Sample every 12th candle (1 hour = 12 M5 candles) — avoids over-sampling
    if (i % 12 !== 0) continue;
    ...
    // Otherwise the engine uses future H4 data (look-ahead bias)
    ...
    // Use raw prices, no rounding (caused pip loss with 4-decimal rounding)
    ...
    // Logic: if TP level is hit, record it. If SL is hit later, check if any TP was already hit.
    // If TP was hit before SL, it's a win at the highest TP reached (exit at that TP price).
    // If SL hit before any TP, it's a loss.
    ...
    for (let j = i + 1; j < maxLook; j++) {
      ...
      if (isLong) {
        ...
        if (c.low <= sl) {
          ...
          if (bestTp) {
            // TP was already hit before SL — it's a win, keep TP exit price
            result = `WIN_${bestTp}`;
          } else {
            // No TP hit — it's a loss
            exitPrice = sl;
            result = 'LOSS';
          }
          break;
    ...
    // If still open at end, close at last candle
    if (result === 'OPEN' && bestTp) {
      result = `WIN_${bestTp}`;
    } else if (result === 'OPEN') {
...
try {
  ...
  for (const t of trades.slice(0, 10)) {
    console.log(`  ${t.entryTime} | ${t.direction} | conf=${t.confidence} | ${t.result} | ${t.pips} pips`);
  }
} catch (err: any) {
