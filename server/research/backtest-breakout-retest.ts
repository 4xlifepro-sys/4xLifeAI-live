...
import { scanBreakoutRetestSignals } from './engine-breakout-retest.js';

// ---------------------------------------------------------------------------
// BACKTEST RUNNER: Breakout + Retest Strategy
//
...
async function main() {
  ...
  console.log('\n===================================================================');
  console.log('RESEARCH: Breakout + Retest Strategy - Isolated Backtest');
  console.log('Timeframes: H4 (direction), M15 (breakout + retest)');
  ...
  for (const pair of FOREX_PAIRS) {
    ...
    const signals = scanBreakoutRetestSignals(pair, m5);
  ...
  if (allTimestamps.length === 0) {
    ...
    row('Breakout+Retest', 'In-sample (1-4m)', inStats);
    row('Breakout+Retest', 'Out-of-sample (5-6m)', outStats);
  }

  fs.default.writeFileSync('backtest-breakout-retest-results.json', JSON.stringify({ perPair, combined, allTrades }, null, 0));
  console.log('\nSaved to backtest-breakout-retest-results.json');
