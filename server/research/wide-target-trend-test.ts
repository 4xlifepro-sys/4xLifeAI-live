...
// ===== STRATEGY: WIDE TARGET TREND CONTINUATION =====
// Key insight: Use 5x SL as TP target so wins are large enough to survive costs
function testWideTargetTrend(pair: string, candles: Candle[], brokerCost: number): Trade[] {
  ...
  for (let i = 200; i < candles.length; i++) {
    if (i - lastSignalIdx < 100) continue;
    ...
    // ===== LONG: Strong uptrend, pullback to EMA50, bounce =====
    const strongUptrend = 
      current.close > ema200Val &&
      ema20Val > ema50Val &&
      ema50Val > ema200Val &&
      ema200[i] > ema200[Math.max(0, i - 20)]; // EMA200 rising

    if (strongUptrend) {
      // Pullback: price touched EMA50 zone
      const nearEMA50 = current.low <= ema50Val * 1.003 && current.close > ema50Val;
      const bounce = current.close > current.open;

      if (nearEMA50 && bounce) {
        const entry = current.close;
        const sl = entry - atrVal * 2; // 2x ATR stop
        const slPips = Math.abs(entry - sl) / pipMult;

        if (slPips < 8 || slPips > 50) continue;

        // TP: 5x SL (wide target for big moves)
        const tp = entry + slPips * 5 * pipMult;
        const tpPips = Math.abs(tp - entry) / pipMult;

        let exitPrice = entry;
        let exitReason = 'OPEN';

        for (let j = i + 1; j < Math.min(i + 500, candles.length); j++) {
    ...
    // ===== SHORT: Strong downtrend, pullback to EMA50, bounce down =====
    const strongDowntrend =
      current.close < ema200Val &&
      ema20Val < ema50Val &&
      ema50Val < ema200Val &&
      ema200[i] < ema200[Math.max(0, i - 20)]; // EMA200 falling

    if (strongDowntrend) {
      const nearEMA50 = current.high >= ema50Val * 0.997 && current.close < ema50Val;
      const bounce = current.close < current.open;
...
console.log('===================================================================');
console.log('STRATEGY #22: WIDE TARGET TREND CONTINUATION');
console.log('Entry: Strong trend (EMA20>50>200), pullback to EMA50, bounce');
console.log('SL: 2x ATR');
console.log('TP: 5x SL (wide target for big moves)');
console.log('Key insight: Wins must be large enough to survive costs');
...
for (const pair of pairs) {
  ...
  const trades = testWideTargetTrend(pair, candles, cost);
...
analyzeResults(allTrades, 'Wide Target Trend (5x SL TP)');
...
console.log('\n===================================================================');
if (avgR > 0 && wr > 50) {
  console.log('✅ WIDE TARGET TREND PASSES');
  console.log(`   Win Rate: ${wr.toFixed(1)}%, Avg R: ${avgR.toFixed(3)}`);
} else if (avgR > 0) {
  console.log('⚠️  MARGINAL — Positive avgR but low win rate');
  console.log(`   Win Rate: ${wr.toFixed(1)}%, Avg R: ${avgR.toFixed(3)}`);
} else {
  console.log('❌ WIDE TARGET TREND FAILS');
  console.log(`   Win Rate: ${wr.toFixed(1)}%, Avg R: ${avgR.toFixed(3)}`);
}
console.log('===================================================================\n');

fs.writeFileSync('wide-target-trend-results.json', JSON.stringify({ allTrades, pairResults }, null, 2));
console.log('Saved to wide-target-trend-results.json');
