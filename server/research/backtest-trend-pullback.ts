...
import { scanTrendPullbackSignals } from './engine-trend-pullback.js';

// ---------------------------------------------------------------------------
// BACKTEST RUNNER: Trend Pullback Strategy
//
...
// Simulate trade with fixed TP levels and SL
...
async function main() {
  ...
  console.log('\n===================================================================');
  console.log('RESEARCH: Trend Pullback Strategy - Isolated Backtest');
  console.log('Timeframes: H4 (direction), H1 (EMA cascade), M15 (signals)');
  console.log('Forex pairs only, isolated research - no production files modified');
  console.log('===================================================================');
  ...
  for (const pair of FOREX_PAIRS) {
    ...
    const signals = scanTrendPullbackSignals(pair, m5);
  ...
  if (allTimestamps.length === 0) {
    console.log('No trades generated - cannot perform walk-forward validation');
  } else {
    ...
    // Summary table
    console.log('\n========================= SUMMARY TABLE =========================');
    console.log('STRATEGY              | PERIOD               | Closed | WinRate | AvgR   | PF     | MaxDD(R) | AvgSL(pips)');
    console.log('----------------------------------------------------------------------------------------------------------');
    const row = (name: string, period: string, s: { closed: number; winRate: number; avgR: number; profitFactor: number; maxDD: number; avgSlPips: number }) =>
      console.log(`${name.padEnd(23)}| ${period.padEnd(21)}| ${String(s.closed).padStart(6)} | ${s.winRate.toFixed(1).padStart(6)}% | ${s.avgR.toFixed(3).padStart(6)} | ${s.profitFactor.toFixed(2).padStart(6)} | ${s.maxDD.toFixed(2).padStart(8)} | ${s.avgSlPips.toFixed(1).padStart(11)}`);
    row('Trend Pullback', 'In-sample (1-4m)', inStats);
    row('Trend Pullback', 'Out-of-sample (5-6m)', outStats);
  }

  fs.default.writeFileSync('backtest-trend-pullback-results.json', JSON.stringify({ perPair, combined, allTrades }, null, 0));
  console.log('\nSaved to backtest-trend-pullback-results.json');
  console.log('NOTE: isolated research backtest - no production files modified.');
}
