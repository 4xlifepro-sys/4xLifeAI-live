...
// Trade specific candlestick patterns: engulfing, pin bars, etc.
function testCandlestickPatterns(pair: string, candles: Candle[], brokerCost: number): Trade[] {
  ...
  for (let i = 50; i < candles.length - 1; i++) {
    ...
    const bullishEngulfing = prevBearish && currentBullish && 
                             currentBody > prevBody &&
                             current.close > prev.open &&
                             current.open < prev.close;

    if (bullishEngulfing && current.close > ema50Val) {
      ...
      for (let j = i + 2; j < Math.min(i + 100, candles.length); j++) {
    ...
    const bearishEngulfing = prevBullish && currentBearish &&
                             currentBody > prevBody &&
                             current.close < prev.open &&
                             current.open > prev.close;

    if (bearishEngulfing && current.close < ema50Val) {
      const entry = current.close;
    ...
    const bullishPinBar = lowerWick > currentBody * 2 && 
                          lowerWick > upperWick * 2 &&
                          currentRange > atrVal * 0.5 &&
                          current.close > ema50Val;

    if (bullishPinBar) {
      const entry = current.close;
      const sl = current.low - atrVal * 0.2;
      const slPips = Math.abs(entry - sl) / pipMult;
    ...
    const bearishPinBar = upperWick > currentBody * 2 &&
    ...
    if (bearishPinBar) {
      const entry = current.close;
      const sl = current.high + atrVal * 0.2;
      const slPips = Math.abs(entry - sl) / pipMult;
...
console.log('===================================================================');
console.log('STRATEGY #25: CANDLESTICK PATTERNS');
console.log('Entry: Bullish/bearish engulfing, pin bars');
console.log('Filter: Must be in direction of EMA50 trend');
console.log('Exit: 2x SL target');
console.log('Key insight: Classic reversal patterns with trend filter');
...
for (const pair of pairs) {
  ...
  const trades = testCandlestickPatterns(pair, candles, cost);
...
analyzeResults(allTrades, 'Candlestick Patterns');
...
console.log('\n===================================================================');
if (avgR > 0 && wr > 50) {
  console.log('✅ CANDLESTICK PATTERNS PASSES');
  console.log(`   Win Rate: ${wr.toFixed(1)}%, Avg R: ${avgR.toFixed(3)}`);
} else if (avgR > 0) {
  console.log('⚠️  MARGINAL — Positive avgR but low win rate');
  console.log(`   Win Rate: ${wr.toFixed(1)}%, Avg R: ${avgR.toFixed(3)}`);
} else {
  console.log('❌ CANDLESTICK PATTERNS FAILS');
  console.log(`   Win Rate: ${wr.toFixed(1)}%, Avg R: ${avgR.toFixed(3)}`);
}
console.log('===================================================================\n');

fs.writeFileSync('candlestick-patterns-results.json', JSON.stringify({ allTrades, pairResults }, null, 2));
console.log('Saved to candlestick-patterns-results.json');
