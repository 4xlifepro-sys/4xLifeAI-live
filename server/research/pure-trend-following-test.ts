...
function testPureTrendFollowing(pair: string, candles: Candle[], brokerCost: number): Trade[] {
  ...
  for (let i = 200; i < candles.length; i++) {
    ...
    // ===== LONG: Price > EMA200, pullback to EMA50, bounce =====
    const trendUp = current.close > ema200Val && ema50Val > ema200Val;
    const pullback = current.low <= ema50Val * 1.002 && current.close > ema50Val;
    const bounce = current.close > current.open;

    if (trendUp && pullback && bounce) {
      const entry = current.close;
      const sl = entry - atrVal * 2; // Wider SL: 2x ATR
      const slPips = Math.abs(entry - sl) / pipMult;

      if (slPips < 5 || slPips > 50) continue;

      // TP: 4x SL (wider target for bigger moves)
      const tp = entry + slPips * 4 * pipMult;

      let exitPrice = entry;
      let exitReason = 'OPEN';

      for (let j = i + 1; j < Math.min(i + 300, candles.length); j++) {
    ...
    // ===== SHORT: Price < EMA200, pullback to EMA50, bounce down =====
    const trendDown = current.close < ema200Val && ema50Val < ema200Val;
    const pullbackShort = current.high >= ema50Val * 0.998 && current.close < ema50Val;
    const bounceDown = current.close < current.open;

    if (trendDown && pullbackShort && bounceDown) {
      const entry = current.close;
...
console.log('===================================================================');
console.log('STRATEGY #19: PURE TREND FOLLOWING');
console.log('Entry: Price > EMA200, pullback to EMA50, bounce candle');
console.log('SL: 2x ATR (wider stop)');
console.log('TP: 4x SL (wider target for bigger moves)');
...
for (const pair of pairs) {
  ...
  const trades = testPureTrendFollowing(pair, candles, cost);
...
analyzeResults(allTrades, 'Pure Trend Following');
...
console.log('\n===================================================================');
if (avgR > 0 && wr > 50) {
  console.log('✅ PURE TREND FOLLOWING PASSES');
  console.log(`   Win Rate: ${wr.toFixed(1)}%, Avg R: ${avgR.toFixed(3)}`);
} else if (avgR > 0) {
  console.log('⚠️  MARGINAL — Positive avgR but low win rate');
  console.log(`   Win Rate: ${wr.toFixed(1)}%, Avg R: ${avgR.toFixed(3)}`);
} else {
  console.log('❌ PURE TREND FOLLOWING FAILS');
  console.log(`   Win Rate: ${wr.toFixed(1)}%, Avg R: ${avgR.toFixed(3)}`);
}
console.log('===================================================================\n');

fs.writeFileSync('pure-trend-following-results.json', JSON.stringify({ allTrades, pairResults }, null, 2));
console.log('Saved to pure-trend-following-results.json');
