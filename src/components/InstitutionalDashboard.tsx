...
interface ClosedSignal {
  ...
  entry: number;
  exit: number;
  result: "win" | "loss" | "breakeven" | "invalid" | "cancelled";
  pips: number;
            ...
            {d.history.map((h, i) => (
              <div className="x4-table__row" key={i}>
                ...
                <span className={`x4-result x4-result--${h.result}`}>
                  {h.result === "win" ? "WIN" : h.result === "loss" ? "LOSS" : h.result === "invalid" ? "INVALID" : h.result === "cancelled" ? "CANCELLED" : "B/E"}
                </span>
...
.x4-signal__time { margin-left: auto; font-size: 10px; }
.x4-signal__progress {
  display: flex;
  align-items: center;
  gap: 8px;
...
.x4-result--cancelled { background: rgba(148,163,184,0.14); color: var(--x4-text-dim); }
